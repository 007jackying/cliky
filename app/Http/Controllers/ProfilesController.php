...
class ProfilesController extends Controller
{
    ...
    {
        if (Input::hasFile('user-avatar')) {
            $currentUser = \Auth::user();
            $photos_path = public_path('/images/profile/'.$currentUser->id.'/avatar');
            if(!is_dir($photos_path))
                mkdir($photos_path, 0777, true);
            $avatar = Input::file('file');
            $filename = md5(rand()).\File::extension($avatar);
            $save_path = storage_path().'/users/id/'.$currentUser->id.'/uploads/images/avatar/';
            $path = $save_path.$filename;
            $public_path = $photos_path.$filename;
